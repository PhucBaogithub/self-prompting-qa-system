...
def full_decode(qid, realqid, question, handler, args, dataobj):
    ...
    source = args.source
    sid = args.sid
    seed = args.pick_demo_seed
    with_restrict = args.with_restrict
    ...
    recall_select_answer_prefix = dataobj.recall_select_answer_template(
    ...
    test_prompt = dataobj.build_input(
        question=question,
        with_restrict=with_restrict,
        demo_way=demo_way,
        in_cot_way=in_cot_way,
    ...
    # ✅ Unified handler
    if hasattr(handler, 'get_output'):
        output = handler.get_output(
            final_input,
            max_tokens_mapping[demo_way],
            temperature=0.0,
            ban_pronoun=False
        )
        ...
        raise ValueError("Handler không hỗ trợ get_output().")
    ...
    processed_pred_first_iter = dataobj.post_process(
        output, demo_way, in_cot_way, with_restrict,
